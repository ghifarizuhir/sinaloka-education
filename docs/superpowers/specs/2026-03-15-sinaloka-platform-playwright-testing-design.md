...
| Theme coverage | Light + dark mode | Both run same tests via Playwright `colorScheme` projects |
...
- **Base URL**: `http://localhost:3000` (matches Vite dev server)
- **Projects**: `light-mode` (default) and `dark-mode` (`colorScheme: 'dark'`)
...
- Mocks `GET /api/auth/me` to return a test admin user
...
| Token refresh | Simulate 401 → verifies automatic token refresh and request retry |
...
| Delete student | Click delete, confirm → student removed from table |
...
| Keyboard shortcuts | Use keyboard to mark attendance → works correctly |
...
No test code changes needed. The `prefers-color-scheme: dark` media query triggers the app's dark mode. If elements become invisible or unclickable due to theme bugs, tests fail naturally.
