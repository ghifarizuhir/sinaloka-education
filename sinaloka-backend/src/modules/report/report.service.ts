...
export class ReportService {
  ...
  async generateFinanceReport(
    ...
    doc.fontSize(12);
    doc.text(`${l.totalIncome}: ${totalIncome} (${payments._count} ${l.transactions})`);
    doc.text(`${l.totalPayouts}: ${totalPayouts} (${payouts._count} ${l.transactions})`);
    doc.text(`${l.totalExpenses}: ${totalExpenses} (${expenses._count} ${l.transactions})`);
    doc.moveDown();
    doc.font('Helvetica-Bold').text(`${l.netProfit}: ${totalIncome - totalPayouts - totalExpenses}`);
    doc.font('Helvetica');
  ...
  async generateStudentProgressReport(
    ...
    for (const a of attendances) {
      ...
      doc.font('Helvetica');
      doc.text(`${l.status}: ${a.status} | ${l.homework}: ${a.homework_done ? l.done : l.notDone}`);
      if (a.session.topic_covered) doc.text(`${l.topic}: ${a.session.topic_covered}`);
  ...
  private groupByMonth(records: { date: Date | null; amount: any }[]): { month: string; amount: number }[] {
  ...
  async getFinancialSummary(institutionId: string, periodStart: Date, periodEnd: Date) {
    ...
    const [revenue, payouts, expenses] = await Promise.all([
      this.prisma.payment.aggregate({
        where: { ...where, status: 'PAID', paid_date: { gte: periodStart, lte: periodEnd } },
        _sum: { amount: true },
        _count: true,
      }),
      this.prisma.payout.aggregate({
    ...
    return {
      ...
      revenue_by_month: this.groupByMonth(monthlyPayments.map(p => ({ date: p.paid_date, amount: p.amount }))),
    };
  }
  ...
  async getExpenseBreakdown(institutionId: string, periodStart: Date, periodEnd: Date) {
    ...
    return {
      ...
      monthly_trend: this.groupByMonth(monthlyExpenses.map(e => ({ date: e.date, amount: e.amount }))),
    };
  }
