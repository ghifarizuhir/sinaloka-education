interface SinalokaLogoProps {
  size?: number;
  className?: string;
}

export function SinalokaLogo({ size = 36, className }: SinalokaLogoProps) {
  const id = `tg-${size}`;
  return (
    <svg
      width={size}
      ...
      viewBox="0 0 512 512"
      className={className}
      style={{ borderRadius: `${Math.round(size * 0.22)}px` }}
    >
      <defs>
      ...
      {size >= 32 && (
        <g fill="#FDE68A">
          <polygon
            points="0,-14 4,-4 14,-4 6,3 8,14 0,8 -8,14 -6,3 -14,-4 -4,-4"
            transform="translate(256, 120) scale(1.6)"
          />
          {size >= 48 && (
            <>
              <polygon
                points="0,-10 3,-3 10,-3 4,2 6,10 0,5 -6,10 -4,2 -10,-3 -3,-3"
                transform="translate(152, 160) scale(0.9)"
              />
              <polygon
                points="0,-10 3,-3 10,-3 4,2 6,10 0,5 -6,10 -4,2 -10,-3 -3,-3"
                transform="translate(365, 148) scale(0.7)"
              />
            </>
          )}
        </g>
