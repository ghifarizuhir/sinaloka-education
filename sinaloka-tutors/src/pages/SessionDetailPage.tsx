...
const statusIcon = {
  P: <CheckCircle2 className="w-4 h-4 text-lime-400" />,
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          ...
          <div className={cn(
            ...
            session.status === 'completed' ? 'bg-lime-500/20 text-lime-400' :
        ...
        <div className="space-y-4">
          {session.topicCovered && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Topik</p>
              <p className="text-white text-sm">{session.topicCovered}</p>
            </div>
          )}
          {session.sessionSummary && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Ringkasan Sesi</p>
      ...
      {!loading && students.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-center">
            <p className="text-lg font-bold text-lime-400">{presentCount}</p>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Hadir</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-center">
            <p className="text-lg font-bold text-orange-400">{lateCount}</p>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Terlambat</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-center">
            <p className="text-lg font-bold text-red-400">{absentCount}</p>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Tidak Hadir</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-500 px-1">Daftar Siswa</h3>
        {loading ? (
          <div className="space-y-3">
            {[1, 2].map((i) => (
          ...
          students.map((student) => (
            <div key={student.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center justify-between">
              <div>
                <h4 className="text-white font-semibold text-sm">{student.name}</h4>
                <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-wider">{student.grade}</p>
              </div>
              <div className="flex items-center gap-2">
                {student.attendance && statusIcon[student.attendance]}
                <span className={cn(
                  'text-xs font-semibold',
                  student.attendance === 'P' ? 'text-lime-400' :
                ...
                {student.homeworkDone && (
                  <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded">HW</span>
                )}
              </div>
            </div>
          ))
          ...
          <div className="bg-zinc-900/50 border border-dashed border-zinc-800 rounded-xl p-6 text-center">
            <p className="text-zinc-500 text-sm">Tidak ada data siswa.</p>
          </div>
        )}
      </div>
    </div>
